...
//获取comment与c_index字段
$result = NULL;
$count  = safe_query("SELECT `comment`, `c_index` FROM `video` WHERE `btih` = UNHEX(?);", &$result, array(
    's',
    $btih
));
if ($count != 1)
...
switch (isset($_REQUEST['action']) ? strtolower(trim($_REQUEST['action'])) : 'all') { //默认为all,在case 'all';default处理
    case "cid": { //********按弹幕号[start,end]获取,计入view,参数为start,end
        ...
        if ($c_count < 1)
            exit('[]'); //是否计入view应取决于action的参数,因此代码应放在case里
        ...
        if (isset($_REQUEST['start'])) {
            ...
            if (0 <= $temp_start and $temp_start <= $cid_end)
                $cid_start = $temp_start;
        }
        if (isset($_REQUEST['end'])) {
            $temp_end = intval(trim($_REQUEST['end']));
            if (0 <= $temp_end and $temp_end <= $cid_end)
                $cid_end = $temp_end; //赋值前$cid_end仍等于上限
        }
    ...
    case "time": { //********按时间[start,end]来获取,计入view,参数为start,end
        ...
        if (isset($_REQUEST['start'])) {
            $temp_start = intval(trim($_REQUEST['start']));
            if (0 <= $temp_start and $temp_start <= $time_end)
                $time_start = $temp_start;
        }
        if (isset($_REQUEST['end'])) {
            $temp_end = intval(trim($_REQUEST['end']));
            if (0 <= $temp_end and $temp_end <= $time_end)
                $time_end = $temp_end; //赋值前$cid_end仍等于上限
        }
    ...
    case "recent": { //********获取指定cid开始到最后一条,不计入view,参数为start
        ...
        if (isset($_REQUEST['start'])) {
            $temp_start = intval(trim($_REQUEST['start']));
            if (0 <= $temp_start and $temp_start <= $cid_end)
                $cid_start = $temp_start; //当然start/end不需要交换了
        }
        //定位并输出弹幕,function substr(字符串,截取掉的开头字符数,输出的剩余字符数)
        $str_start = ($cid_start == 0) ? 0 : ($c_index[$cid_start - 1][2]); //省去的开头部分的长度,即剩余串第一个字符的下标
        $str_end   = $c_index[$cid_end][2]; //剩余串最后字符的下标,差值是剩余串长(含结尾逗号)

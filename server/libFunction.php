...
{
    $result = NULL;
    $count  = safe_query('SELECT `uid` FROM `user` WHERE `btih` = ?;', &$result, array(
        'i',
        $uid
    ));
    if ($count === 1)
...
{
    $result = NULL;
    $count  = safe_query('SELECT `uid` FROM `video` WHERE `btih` = unhex(?);', &$result, array(
        's',
        $btih
    ));
    if ($count === 1)
...
function checkCookie()
{
    ...
    $result = NULL;
    $count  = safe_query('SELECT * FROM `user` WHERE `uid` = ?;', &$result, array(
        'i',
        $uid
    ));
    if ($count != 1)
        die(json_err('cookie_invalid', -1, 'Error: Invalid Cookie')); //返回空
    //!= == >= 代表作为数字比较
...
{
    $blackhole = NULL;
    $count     = safe_query("UPDATE `user` SET `point` = `point` + ?, `time` = `time` + ? WHERE `uid` = ?;", &$blackhole, array(
        'iii',
        $point,
        $delay,
        $uid
    ...
    if ($count != 1)
        die(json_err('freezing_error', -1, 'Error: Freezing Failed')); //返回空
    return true;
}
?>

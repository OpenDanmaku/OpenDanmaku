...
//只有这里因为需要写cookie和session涉及延迟html输出
//getVcode.php也不涉及,因为用imagepng输出
//而这里除非出错退出,否则在session操作和setcookie()之前,没有echo或var_dump输出
...
if (isset($_COOKIE['uid'])) { //不能使用getCookie或者checkCookie来判断有无cookie!!!
    ...
    $blackhole = NULL;
    $count     = safe_query('UPDATE `user` SET `status` = 0 WHERE `uid` = ?;', &$blackhole, array(
        'i',
        intval($_COOKIE['uid'])
    ));
    if ($count != 1)
...
//保存新账号到数据库
$blackhole = NULL;
$count     = safe_query('INSERT INTO `user` VALUES (?, ?, ?, ?, ?);', &$blackhole, array(
    'iiiii',
    $uid,
    $key,
    $time,

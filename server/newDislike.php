...
$count  = safe_query("SELECT `c_index`, `dislike`, `d_index` FROM `video` WHERE `btih` = UNHEX(?);", &$result, array(
    's',
    $btih
)); //http://stackoverflow.com/questions/1747894/
if ($count != 1)
    die(json_err('btih_unavailable', -1, 'Error: Video Not Yet Exists, Do You Want to Create It?')); //返回空
...
if (!isset($c_index[$cid]))
    die(json_err('cid_unavailable', -1, 'Error: Comment Item Not Yet Exists')); //那条弹幕不存在
if (!isset($dislike[$cid]))
    $dislike[$cid] = array(); //强制储存为一个数组,防止作为一个值储存,$cid始终是字符串
//取键值
$this_uid = $c_index[$cid][0];
if ($this_uid == $uid)
    die(json_err('uid_invalid', -1, 'Error: You Cannot Dislike Yourself!')); //不许自己差评自己
...
$blackhole = NULL;
$count     = safe_query("UPDATE `video` SET `dislike` = ?, `d_index` = ? WHERE `btih` = UNHEX(?);", &$blackhole, array(
    'sss',
    $dislike,
    $d_index,
    $btih

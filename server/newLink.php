...
if (count($head_comma)<3 ) die(json_err('btih_incorrect',-1,'Error: Link is Not Valid'));
...
//获取linkage和l_index
$result_1=NULL;
if(1!=safe_query("SELECT `linkage`, `l_index` FROM `video` WHERE `btih` = UNHEX(?);",&$result_1, array('s',$btih_1))) 
...
$result_2=NULL;
if(1!=safe_query("SELECT `linkage`, `l_index` FROM `video` WHERE `btih` = UNHEX(?);",&$result_2, array('s',$btih_2))) 
	die(json_err('btih_unavailable',-1,'Error: Video Not Yet Exists, Do You Want to Create It?'));//无返回值
...
//编辑键值
//如果有先例,不加分,但仍硬直
if(isset($linkage_1[$key_1]) and isset($linkage_2[$key_2])) $const_PointNewLink = 0;
//var_dump($linkage_1);
//echo '<br>';
//var_dump($linkage_2);
//echo '<br>';
...
$count=safe_query("UPDATE `video` SET `linkage` = ?, `l_index` = ? WHERE `btih` = UNHEX(?);",
		&$blackhole, array('sss', $linkage_1, $l_index_1, $btih_1));
$blackhole=NULL;
$count=safe_query("UPDATE `video` SET `linkage` = ?, `l_index` = ? WHERE `btih` = UNHEX(?);",
		&$blackhole, array('sss', $linkage_2, $l_index_2, $btih_2));

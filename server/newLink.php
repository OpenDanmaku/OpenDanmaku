...
$count  = intval(trim($head_comma[2])); //偏移量的计数,下面与shift过后的数组$linkage的元素数比较
if (count($linkage) < $count)
    die(json_err('link_incomplete', -1, 'Error: Linkage is Not Complete.')); //正常应该相等,允许大于
...
//获取linkage和l_index
$result_1 = NULL;
if (1 != safe_query("SELECT `linkage`, `l_index` FROM `video` WHERE `btih` = UNHEX(?);", &$result_1, array(
    's',
    $btih_1
)))
    die(json_err('btih_unavailable', -1, 'Error: Video Not Yet Exists, Do You Want to Create It?')); //无返回值
$linkage_1 = json_decode($result_1[0]['linkage'], true); //json->array(rather than object)
$l_index_1 = json_decode($result_1[0]['l_index'], true); //json->array(rather than object)

$result_2 = NULL;
if (1 != safe_query("SELECT `linkage`, `l_index` FROM `video` WHERE `btih` = UNHEX(?);", &$result_2, array(
    's',
    $btih_2
)))
    die(json_err('btih_unavailable', -1, 'Error: Video Not Yet Exists, Do You Want to Create It?')); //无返回值
$linkage_2 = json_decode($result_2[0]['linkage'], true); //json->array(rather than object)
$l_index_2 = json_decode($result_2[0]['l_index'], true); //json->array(rather than object)

//编辑键值
//如果有先例,不加分,但仍硬直
if (isset($linkage_1[$key_1]) and isset($linkage_2[$key_2]))
    $const_PointNewLink = 0;
//var_dump($linkage_1);
//echo '<br>';
//var_dump($linkage_2);
//echo '<br>';
...
if (!isset($linkage_1[$key_1]))
    $linkage_1[$key_1] = array(); //如果不存在btih,强制储存为一个数组,防止作为一个值储存
if (!isset($linkage_2[$key_2]))
    $linkage_2[$key_2] = array(); //当然,对应的另一个link也不存在,但是我还是要独立处理
//var_dump($linkage_1);
//echo '<br>';
//var_dump($linkage_2);
//echo '<br>';
...
//任意一个没提交
if (!in_array($uid, $linkage_1[$key_1]))
    $linkage_1[$key_1][] = $uid; //Add Elements to Multidimensional Array
if (!in_array($uid, $linkage_2[$key_2]))
    $linkage_2[$key_2][] = $uid; //http://stackoverflow.com/a/16308305
//var_dump($linkage_1);
//echo '<br>';
//var_dump($linkage_2);
//echo '<br>';
...
$l_index_2[$key_2] = count($linkage_2[$key_2]); //这个自然是一个值,所以无所谓
...
$blackhole = NULL;
$count     = safe_query("UPDATE `video` SET `linkage` = ?, `l_index` = ? WHERE `btih` = UNHEX(?);", &$blackhole, array(
    'sss',
    $linkage_1,
    $l_index_1,
    $btih_1
